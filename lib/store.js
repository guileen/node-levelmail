...
function saveMessage(req, body) {
    ...
    levelIncr(K_MSG_SEQ, function(err, seq) {
            leveldown.put(makeUInt32Key(K_MSG, seq), JSON.stringify({
                        from: req.from,
                        to: req.to,
                        time: new Date(),
                        body: body
...
function listAccounts(startAccount, endAccount, limit, reverse, callback) {
    range(K_ACCOUNT_MAIL_COUNT + startAccount, K_ACCOUNT_MAIL_COUNT + endAccount, reverse, limit, function(err, results) {
            if(err)return callback(err);
...
function listMessageAll(startSeq, toSeq, reverse, limit, callback) {
    range(makeUInt32Key(K_MSG, startSeq), makeUInt32Key(K_MSG, toSeq), reverse, limit, function(err, results) {
            console.log('results1', results)
            if(err) return callback(err);
            callback(null, results.map(function(item) {
                        return item && {
                            seq: item[0].readUInt32BE(Buffer.byteLength(K_MSG)),
                            message: JSON.parse(item[1].toString('utf-8'))
                        }
            }))
    })
}
